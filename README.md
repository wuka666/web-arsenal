...
> **139 条素材 + 23 条方案**，每条配可运行的 Demo、可调参数和一段完整提示词。
...
| `assets/demos/` | 162 个可双击运行的演示（原生 HTML/CSS/JS） |
| `assets/库/starflow.js` | 第三方粒子引擎（MIT，供 3D 完整版调用） |
| `data/素材.js` | 素材数据源（139 条，`window.WEB_ARSENAL`） |
...
- 139 条素材**逐条标注「来源 + 分析日期」**，字段就写在 `data/素材.js` 里，可追溯
