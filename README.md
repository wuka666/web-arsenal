...
> 把「看到的好效果」变成「能跑、能调、能直接喂给 AI」的技法。
> **273 条素材 + 46 条方案**，每条配可运行的 Demo、可调参数和一段完整提示词。
...
| `assets/demos/` | 335 个可双击运行的演示（原生 HTML/CSS/JS） |
| `assets/库/starflow.js` | 第三方粒子引擎（MIT，供 3D 完整版调用） |
| `data/素材.js` | 素材数据源（273 条，`window.WEB_ARSENAL`） |
| `data/方案.js` | 方案数据源（46 条，`window.WEB_SCHEMES`） |
| `scripts/` | 入库与体检工具（都要在根目录运行）：`check_all.js` 一条命令跑全库基线；`check_skeleton.js` **跨条目骨架雷同比对**（规矩 21 第 4 项，`check_all.js` 查不出这层）；`check_adapt.js` 适配端实测（`--materials` 扫素材库；375×812 真机视口量横向溢出与并排多栏）；`rebuild_tags.js` 按四维词表全量重组标签（`--dry` 可干跑）；`test_filters.js` 分级筛选回归测试（无头浏览器注入点击）；`sync_code.js` / `sync_scheme_code.js` 代码↔demo 同步；`verify_entry.js` 条目校验（清单在同目录 `new_entries.js`）；`check_palette.js` 配色对齐；`check_links.js` 零外链；`check_param_types.js` 参数类型/用法冲突；`shot_demo.js` headless 截图 |
...
- **四维分级筛选**：适配端（通用 / PC 端 / 移动端）· 元素（动作 / 输入 / 导航 / 反馈 / 数据 / 容器布局 / 媒体 / 文字 / 动效 / 背景氛围）· 风格（13 类）· 场景（7 类）
  ...
  - 另有 46 条整站方案（每套独立版式 + 独立色系，非换肤）

---

## 来源与原创声明

- 273 条素材**逐条标注来源**（写在 `data/素材.js` 的 `来源` 字段，可追溯），按宪法「规矩 19」五分类：`① 具体网站`（保留域名与分析日期）、`② 抖音`（统一写作「来自于抖音」）、`③ 自研`、`④ 机制参考自…已换题重推`、`⑤ 方案库拆解`
- 所有代码均为观察交互手法后的**零依赖自写**（原生 HTML/CSS/JS，未复制任何网站源码、字体、图片或商业素材）
- 借鉴对象包括：ReactBits（源 `DavidHDev/react-bits`，**MIT + Commons Clause**，已逐条标注；仅借机制用原生 JS 重写，**不搬运其源码、不再分发其组件本体**）、Stripe / Apple / stateofaidesign / Awwwards 等官网的交互手法，以及视频资料中的通用交互模式
...
node scripts/check_skeleton.js   # 跨条目骨架雷同比对（规矩 21 第 4 项）—— check_all.js 查不出这层
node scripts/shot_demo.js "assets/demos/xxx.html"   # headless 截图，确认渲染非空白（改过 demo 时）
```

`check_all.js` 与 `check_skeleton.js` 不合格均退出码非 0，可直接挂 CI。改动 demo 后记得跑 `sync_code.js`（素材）/ `sync_scheme_code.js`（方案），保证「代码 = demo 逐字节」。

> ⚠️ **两条命令都过 ≠ 入库合格**：它们查的分别是「单条自身」与「方案之间的骨架」。**「新条目的原站是否已被现有条目覆盖」（同源重复）仍需人工确认** —— 先 `grep` 一下 `data/*.js` 的 `来源` / `参考站` 里有没有同一个域名。
