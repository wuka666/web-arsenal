...
> **248 条素材 + 36 条方案**，每条配可运行的 Demo、可调参数和一段完整提示词。
...
| `assets/demos/` | 304 个可双击运行的演示（原生 HTML/CSS/JS） |
| `assets/库/starflow.js` | 第三方粒子引擎（MIT，供 3D 完整版调用） |
| `data/素材.js` | 素材数据源（248 条，`window.WEB_ARSENAL`） |
| `data/方案.js` | 方案数据源（36 条，`window.WEB_SCHEMES`） |
...
- **四维分级筛选**：适配端（通用 / PC 端 / 移动端）· 元素（动作 / 输入 / 导航 / 反馈 / 数据 / 容器布局 / 媒体 / 文字 / 动效 / 背景氛围）· 风格（13 类）· 场景（7 类）
  ...
  - 另有 36 条整站方案（每套独立版式 + 独立色系，非换肤）

---

## 来源与原创声明

- 248 条素材**逐条标注来源**（写在 `data/素材.js` 的 `来源` 字段，可追溯），按宪法「规矩 19」五分类：`① 具体网站`（保留域名与分析日期）、`② 抖音`（统一写作「来自于抖音」）、`③ 自研`、`④ 机制参考自…已换题重推`、`⑤ 方案库拆解`
