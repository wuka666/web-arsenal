...
> **105 条素材 + 22 条方案**，每条配可运行的 Demo、可调参数和一段完整提示词。
...
| `assets/demos/` | 128 个可双击运行的演示（原生 HTML/CSS/JS） |
| `assets/库/starflow.js` | 第三方粒子引擎（MIT，供 3D 完整版调用） |
| `data/素材.js` | 素材数据源（105 条，`window.WEB_ARSENAL`） |
| `data/方案.js` | 方案数据源（22 条，`window.WEB_SCHEMES`） |
...
- **六大分类**：文字动画 / 背景 / 动效 / 组件 / 布局骨架 / AI 反馈；另有 22 条整站方案（共享骨架 + 换肤）

---

## 来源与原创声明

- 105 条素材**逐条标注「来源 + 分析日期」**，字段就写在 `data/素材.js` 里，可追溯
- 所有代码均为观察交互手法后的**原创复刻 / 自写**，零依赖重写，未复制任何网站源码、字体、图片或商业素材
- 借鉴对象包括：ReactBits（MIT，已逐条标注）、Stripe / Apple / stateofaidesign 等官网的交互手法，以及视频资料中的通用交互模式
